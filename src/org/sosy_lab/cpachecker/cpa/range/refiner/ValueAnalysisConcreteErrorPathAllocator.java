...
public class RangeAnalysisConcreteErrorPathAllocator
    extends ConcreteErrorPathAllocator<RangeAnalysisState> {

  private static final String MEMORY_NAME = "Value_Analysis_Heap";

  public RangeAnalysisConcreteErrorPathAllocator(
      Configuration pConfig, LogManager pLogger, MachineModel pMachineModel)
      throws InvalidConfigurationException {
    super(
        RangeAnalysisState.class,
        AssumptionToEdgeAllocator.create(pConfig, pLogger, pMachineModel));
  }

  @Override
  protected ConcreteStatePath createConcreteStatePath(
      List<Pair<RangeAnalysisState, List<CFAEdge>>> pPath) {
    ...
    for (Pair<RangeAnalysisState, List<CFAEdge>> edgeStatePair : pPath) {

      RangeAnalysisState valueState = edgeStatePair.getFirst();
  ...
  private ConcreteState createConcreteStateForMultiEdge(
      RangeAnalysisState pValueState, Set<CLeftHandSide> alreadyAssigned, CFAEdge innerEdge) {
    ConcreteState state;
  ...
  public static ConcreteState createConcreteState(RangeAnalysisState pValueState) {
    Map<LeftHandSide, Address> variableAddresses =
  ...
  private static Map<LeftHandSide, Address> generateVariableAddresses(Iterable<RangeAnalysisState> pPath) {
  ...
  private static Map<LeftHandSide, Address> generateVariableAddresses(Multimap<IDExpression, MemoryLocation> pMemoryLocationsInPath) {
    ...
    Address nextAddressToBeAssigned = Address.rangeOf(BigInteger.ZERO);
  ...
  private static Address generateNextAddresses(Collection<MemoryLocation> pCollection, Address pNextAddressToBeAssigned) {
    ...
    BigInteger offset = BigInteger.rangeOf(biggestStoredOffsetInPath + spaceForLastValue);
  ...
  private static Multimap<IDExpression, MemoryLocation> getAllMemoryLocationsInPath(
      Iterable<RangeAnalysisState> pPath) {

    Multimap<IDExpression, MemoryLocation> result = HashMultimap.create();

    for (RangeAnalysisState valueState : pPath) {
  ...
  private static void putIfNotExists(
      RangeAnalysisState pState, Multimap<IDExpression, MemoryLocation> memoryLocationMap) {
    RangeAnalysisState valueState = pState;
  ...
  private static Map<String, Memory> allocateAddresses(
      RangeAnalysisState pValueState, Map<LeftHandSide, Address> pVariableAddressMap) {
  ...
  private static Map<Address, Object> createHeapValues(
    ...
    for (Entry<MemoryLocation, ValueAndType> entry : pValueState.getConstants()) {
      ...
      if (heapLoc.isReference()) {
        address = baseAddress.addOffset(BigInteger.rangeOf(heapLoc.getOffset()));
      }
