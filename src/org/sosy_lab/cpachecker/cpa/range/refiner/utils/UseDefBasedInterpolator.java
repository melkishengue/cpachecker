...
public class UseDefBasedInterpolator {
   ...
   * and their respective {@link RangeAnalysisInterpolant}s.
   *
   * @return the (ordered) list of {@link ARGState}s and their respective {@link RangeAnalysisInterpolant}s
   */
  public List<Pair<ARGState, RangeAnalysisInterpolant>> obtainInterpolants() {
    Map<ARGState, Collection<ASimpleDeclaration>> useDefSequence = useDefRelation.getExpandedUses(slicedPrefix);
    RangeAnalysisInterpolant trivialItp = RangeAnalysisInterpolant.FALSE;

    // reverse order!
    List<Pair<ARGState, RangeAnalysisInterpolant>> interpolants = new ArrayList<>();
    ...
    while (iterator.hasNext()) {
      ...
      RangeAnalysisInterpolant interpolant = uses.isEmpty()
          ? trivialItp
      ...
      if (interpolant != trivialItp) {
        trivialItp = RangeAnalysisInterpolant.TRUE;
      }
    }
   ...
   * to their respective {@link RangeAnalysisInterpolant}s.
   *
   * @return the (ordered) mapping from {@link ARGState}s to their respective {@link RangeAnalysisInterpolant}s
   */
  public Map<ARGState, RangeAnalysisInterpolant> obtainInterpolantsAsMap() {

    Map<ARGState, RangeAnalysisInterpolant> interpolants = new LinkedHashMap<>();
    for(Pair<ARGState, RangeAnalysisInterpolant> itp : obtainInterpolants()) {
   ...
   */
  private RangeAnalysisInterpolant createInterpolant(Collection<ASimpleDeclaration> uses) {
    ...
    return new RangeAnalysisInterpolant(PathCopyingPersistentTreeMap.copyOf(useDefInterpolant));
  }
  ...
  private static class MemoryLocationCreator implements CTypeVisitor<List<MemoryLocation>, IllegalArgumentException> {
    ...
    private List<MemoryLocation> createSingleMemoryLocation(final long pSize) {
      if (withinComplexType) {
        List<MemoryLocation> memory = Collections.singletonList(MemoryLocation.rangeOf(qualifiedName, currentOffset));
      ...
      return Collections.singletonList(MemoryLocation.rangeOf(qualifiedName));
    }
