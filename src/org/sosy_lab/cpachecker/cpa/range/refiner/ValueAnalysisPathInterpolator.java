...
import org.sosy_lab.cpachecker.cpa.range.refiner.utils.UseDefBasedInterpolator;
import org.sosy_lab.cpachecker.cpa.range.refiner.utils.RangeAnalysisEdgeInterpolator;
...
public class RangeAnalysisPathInterpolator
    extends GenericPathInterpolator<RangeAnalysisState, RangeAnalysisInterpolant> {
  ...
  private final RangeAnalysisInterpolantManager interpolantManager;

  public RangeAnalysisPathInterpolator(
      final FeasibilityChecker<RangeAnalysisState> pFeasibilityChecker,
    ...
    super(new RangeAnalysisEdgeInterpolator(pFeasibilityChecker,
    ...
    pConfig.inject(this);
    cfa = pCfa;
    interpolantManager = RangeAnalysisInterpolantManager.getInstance();
  }

  @Override
  public Map<ARGState, RangeAnalysisInterpolant> performInterpolation(
      final ARGPath errorPath,
      final RangeAnalysisInterpolant interpolant
    ...
    if (performEdgeBasedInterpolation) {
      ...
      Map<ARGState, RangeAnalysisInterpolant> interpolants =
   ...
   */
  private Map<ARGState, RangeAnalysisInterpolant> performPathBasedInterpolation(ARGPath errorPathPrefix) {
    ...
    Map<ARGState, RangeAnalysisInterpolant> interpolants = new UseDefBasedInterpolator(
        errorPathPrefix,
        useDefRelation,
    ...
    int size = 0;
    for(RangeAnalysisInterpolant itp : interpolants.ranges()) {
  ...
  public Multimap<CFANode, MemoryLocation> determinePrecisionIncrement(ARGPath errorPath)
    ...
    Map<ARGState, RangeAnalysisInterpolant> itps =
        performInterpolation(errorPath, interpolantManager.createInitialInterpolant());

    for (Map.Entry<ARGState, RangeAnalysisInterpolant> itp : itps.entrySet()) {
  ...
  private void addToPrecisionIncrement(
      ...
      final RangeAnalysisInterpolant itp
  ) {
