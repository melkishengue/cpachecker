...
import org.sosy_lab.cpachecker.cpa.range.RangeAnalysisTransferRelation.rangeTransferOptions;
import org.sosy_lab.cpachecker.cpa.range.refiner.RangeAnalysisConcreteErrorPathAllocator;
import org.sosy_lab.cpachecker.cpa.range.symbolic.ConstraintsStrengthenOperator;
import org.sosy_lab.cpachecker.cpa.range.symbolic.SymbolicRangeAnalysisPrecisionAdjustment;
import org.sosy_lab.cpachecker.cpa.range.symbolic.SymbolicRangeAnalysisPrecisionAdjustment.SymbolicStatistics;
import org.sosy_lab.cpachecker.cpa.range.symbolic.SymbolicValueAssigner;
...
public class RangeAnalysisCPA extends AbstractCPA
  ...
  @Option(secure=true, name="merge", toUppercase=true, values={"SEP", "JOIN"},
      description="which merge operator to use for RangeAnalysisCPA")
  ...
  @Option(
      ...
      description = "which stop operator to use for RangeAnalysisCPA")
  ...
  public static CPAFactory factory() {
    return AutomaticCPAFactory.forType(RangeAnalysisCPA.class);
  }

  private VariableTrackingPrecision precision;
  private final RangeAnalysisCPAStatistics statistics;
  ...
  private RangeAnalysisConcreteErrorPathAllocator errorPathAllocator;
  ...
  private RangeAnalysisCPA(Configuration config, LogManager logger,
      ShutdownNotifier pShutdownNotifier, CFA cfa) throws InvalidConfigurationException {
    super(DelegateAbstractDomain.<RangeAnalysisState>getInstance(), null);
    this.config           = config;
    this.logger           = logger;
    this.shutdownNotifier = pShutdownNotifier;
    this.cfa              = cfa;

    config.inject(this, RangeAnalysisCPA.class);

    precision           = initializePrecision(config, cfa);
    statistics          = new RangeAnalysisCPAStatistics(this, config);
    writer = new StateToFormulaWriter(config, logger, shutdownNotifier, cfa);
    errorPathAllocator = new RangeAnalysisConcreteErrorPathAllocator(config, logger, cfa.getMachineModel());
  ...
  private Multimap<CFANode, MemoryLocation> restoreMappingFromFile(CFA pCfa) {
    ...
    for (String currentLine : contents) {
      if (currentLine.trim().isEmpty()) {
        ...
        mapping.put(location, MemoryLocation.rangeOf(currentLine));
      }
    }
  ...
  private CFANode getDefaultLocation(Map<Integer, CFANode> idToCfaNode) {
    return idToCfaNode.ranges().iterator().next();
  }
  ...
  @Override
  public RangeAnalysisTransferRelation getTransferRelation() {
    return new RangeAnalysisTransferRelation(
        logger,
        cfa,
        transferOptions,
        unknownValueHandler,
  ...
  public AbstractState getInitialState(CFANode pNode, StateSpacePartition pPartition) {
    return new RangeAnalysisState(cfa.getMachineModel());
  }
  ...
  public PrecisionAdjustment getPrecisionAdjustment() {
    if (unknownValueStrategy.equals(UnknownValueStrategy.INTRODUCE_SYMBOLIC)) {
      ...
      return new SymbolicRangeAnalysisPrecisionAdjustment(
          statistics,
          cfa,
      ...
      return new RangeAnalysisPrecisionAdjustment(
  ...
  public Reducer getReducer() {
    return new RangeAnalysisReducer();
  }
