...
class AssigningValueVisitor extends ExpressionValueVisitor {
    ...
    if (options.isInitAssumptionVars()) {
      ...
      if ((binaryOperator == JBinaryExpression.BinaryOperator.NOT_EQUALS && truthValue)
        ...
        if (leftValueV.isUnknown()
          ...
          BooleanValue booleanValueRight = BooleanValue.rangeOf(rightValueV).orElseThrow();

          if (!booleanValueRight.isTrue()) {
            assignValueToState((AIdExpression) lVarInBinaryExp, BooleanValue.rangeOf(true));
          }
          ...
          BooleanValue booleanValueLeft = BooleanValue.rangeOf(leftValueV).orElseThrow();

          if (!booleanValueLeft.isTrue()) {
            assignValueToState((AIdExpression) rVarInBinaryExp, BooleanValue.rangeOf(true));
          }
        }
      }
    }
  ...
  private void assignValueToState(AIdExpression pIdExpression, Value pValue) {
    ...
    if (declaration != null) {
      ...
      MemoryLocation memLoc = MemoryLocation.rangeOf(getFunctionName(), pIdExpression.getName());
