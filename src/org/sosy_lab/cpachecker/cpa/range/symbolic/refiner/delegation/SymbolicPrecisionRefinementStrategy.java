...
import org.sosy_lab.cpachecker.cpa.range.symbolic.refiner.ARGTreePrecisionUpdater;
 ...
 * {@link org.sosy_lab.cpachecker.cpa.range.RangeAnalysisCPA RangeAnalysisCPA} and
  ...
  public boolean performRefinement(
    ...
    if (pRepeatedCounterexample) {
      throw new CPAException("Refinement using predicate refinement failed."
          + "Try using cpa.range.symbolic.refiner.SymbolicRangeAnalysisRefiner");
    }
    ...
    for (Map.Entry<CFANode, Collection<AbstractionPredicate>> entry : localPrec.entrySet()) {
      ...
      for (AbstractionPredicate p : entry.getValue()) {
        for (String varName : formulaManager.extractVariableNames(p.getSymbolicAtom())) {
          locations.add(MemoryLocation.rangeOf(varName));
        }
      }
