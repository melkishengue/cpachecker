 ...
 */
package org.sosy_lab.cpachecker.cpa.range.symbolic.refiner.interpolant;
...
public final class SymbolicInterpolant
  ...
  static final SymbolicInterpolant FALSE =
      new SymbolicInterpolant((RangeAnalysisInterpolant) null, null);

  private final RangeAnalysisInterpolant valueInterpolant;
  private final ConstraintsInformation constraintsInformation;

  private SymbolicInterpolant() {
    valueInterpolant = RangeAnalysisInterpolant.createInitial();
    constraintsInformation = ConstraintsInformation.EMPTY;
  }

  public SymbolicInterpolant(
      final RangeAnalysisInformation pValueInfo, final ConstraintsInformation pConstraints) {
    checkNotNull(pValueInfo);
    checkNotNull(pConstraints);
    valueInterpolant = new RangeAnalysisInterpolant(pValueInfo.getAssignments());
    constraintsInformation = pConstraints;
  }

  private SymbolicInterpolant(
      final RangeAnalysisInterpolant pValueInterpolant, final ConstraintsInformation pConstraints) {
  ...
  public ForgettingCompositeState reconstructState() {
    final RangeAnalysisState values = valueInterpolant.reconstructState();
  ...
  public SymbolicInterpolant join(SymbolicInterpolant pOther) {
    RangeAnalysisInterpolant newValueInterpolant = valueInterpolant.join(pOther.rangeInterpolant);
  ...
  public boolean equals(Object o) {
    ...
    return Objects.equals(constraintsInformation, that.constraintsInformation)
        && Objects.equals(valueInterpolant, that.rangeInterpolant);
  }
