...
public class ExpressionTransformer {
  ...
  protected RangeAnalysisState valueState;

  public ExpressionTransformer(String pFunctionName, RangeAnalysisState pValueState) {
  ...
  private MemoryLocation getMemoryLocation(ASimpleDeclaration pDeclaration) {
    if (pDeclaration instanceof ADeclaration && ((ADeclaration) pDeclaration).isGlobal()) {
      return MemoryLocation.rangeOf(pDeclaration.getName());

    } else {
      return MemoryLocation.rangeOf(functionName, pDeclaration.getName());
    }
  }
