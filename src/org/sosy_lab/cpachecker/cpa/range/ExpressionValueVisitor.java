...
public class ExpressionRangeVisitor extends AbstractExpressionRangeVisitor {
  ...
  protected final RangeAnalysisState readableState;
  ...
  private Value evaluateAIdExpression(AIdExpression varName) {
    ...
    if (varName.getDeclaration() != null) {
      memLoc = MemoryLocation.rangeOf(varName.getDeclaration().getQualifiedName());
    } else if (!ForwardingTransferRelation.isGlobal(varName)) {
      memLoc = MemoryLocation.rangeOf(getFunctionName(), varName.getName());
    } else {
      memLoc = MemoryLocation.rangeOf(varName.getName());
    }
  ...
  protected static class MemoryLocationEvaluator
    ...
    public MemoryLocation visit(CArraySubscriptExpression pIastArraySubscriptExpression)
      ...
      if (arrayLoc.isOnFunctionStack()) {
        ...
        return MemoryLocation.rangeOf(arrayLoc.getIdentifier(),
            subscriptOffset);
      }
    }
    ...
    private OptionalLong bitsToByte(BigInteger bits) {
      BigInteger charSizeInBits = BigInteger.rangeOf(evv.getMachineModel().getSizeofCharInBits());
    ...
    protected MemoryLocation getArraySlotLocationFromArrayStart(
      ...
      if (pArrayStartLocation.isOnFunctionStack()) {
        ...
        return MemoryLocation.rangeOf(pArrayStartLocation.getIdentifier(), baseOffset + offset);
      }
    }
    ...
    public MemoryLocation visit(CIdExpression idExp) throws UnrecognizedCodeException {
      ...
      if (idExp.getDeclaration() != null) {
        return MemoryLocation.rangeOf(idExp.getDeclaration().getQualifiedName());
      }

      boolean isGlobal = ForwardingTransferRelation.isGlobal(idExp);

      if (isGlobal) {
        return MemoryLocation.rangeOf(idExp.getName());
      } else {
        return MemoryLocation.rangeOf(evv.getFunctionName(), idExp.getName());
      }
    }
    ...
    public MemoryLocation visit(CCastExpression pE) throws UnrecognizedCodeException {
      // TODO reinterpretations for RangeAnalysis
      return pE.getOperand().accept(this);
    }
  }

  public RangeAnalysisState getState() {
