...
@Options(prefix = "cpa.statement")
public class StatementStateFactory {

  private final StatementState[] states;
  ...
  public StatementStateFactory(CFA pCfa, AnalysisDirection pLocationType, Configuration config)
    ...
    states = new StatementState[maxNodeNumber + 1];
    for (CFANode node : allNodes) {
      StatementState state = createLocationState(node);
      states[node.getNodeNumber()] = state;
    }
  }

  public StatementState getState(CFANode node) {
  ...
  private StatementState createLocationState(CFANode node) {
    return new StatementState(node, followFunctionCalls);
  }
}
