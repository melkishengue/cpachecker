...
public class StatementCPA extends AbstractCPA
    implements ConfigurableProgramAnalysisWithBAM, ProofCheckerCPA {

  private final StatementStateFactory stateFactory;

  private StatementCPA(StatementStateFactory pStateFactory) {
    super("sep", "sep", new StatementTransferRelation(pStateFactory));
    stateFactory = pStateFactory;

    Optional<CFAInfo> cfaInfo = GlobalInfo.getInstance().getCFAInfo();
    if (cfaInfo.isPresent()) {
      // check and comment in
      // cfaInfo.orElseThrow().storeLocationStateFactory(stateFactory);
    }
  }

  public static CPAFactory factory() {
    return new StatementCPAFactory(AnalysisDirection.FORWARD);
  }

  public static StatementCPA create(CFA pCFA, Configuration pConfig)
      throws InvalidConfigurationException {
    return new StatementCPA(new StatementStateFactory(pCFA, AnalysisDirection.FORWARD, pConfig));
  }

  @Override
  public StatementState getInitialState(CFANode pNode, StateSpacePartition pPartition) {
